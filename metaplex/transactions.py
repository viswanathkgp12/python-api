...
async def deploy(api_endpoint, source_account, name, symbol, fees):
    # Initalize Client
    async with AsyncClient(api_endpoint) as client:
        ...
        min_rent_reseponse = await client.get_minimum_balance_for_rent_exemption(MINT_LAYOUT.sizeof())  # type: ignore
...
async def topup(api_endpoint, sender_account, to, amount=None):
    """
...
async def send(
    api_endpoint, source_account, contract_key, sender_key, dest_key, private_key
):
    """
    ...
    async with AsyncClient(api_endpoint) as client:
        ...
        token_pda_acct_info = await client.get_account_info(token_pda_address)
        if token_pda_acct_info["result"]["value"] is None:
            raise Exception
...
async def burn(api_endpoint, contract_key, owner_key, private_key):
    """
