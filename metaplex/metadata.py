...
async def get_metadata(client: AsyncClient, mint_key, retries=5):
    try:
        metadata_account = get_metadata_account(mint_key)
        acct_info = await client.get_account_info(metadata_account)
        ...
        if retries > 0:
            ...
            metadata = await get_metadata(client, mint_key, retries - 1)
            return metadata
